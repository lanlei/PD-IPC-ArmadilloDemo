...
void Box::setFromBounding(const QVector3D min, const QVector3D max)
{
	...
	points.data()[9] = max[0];
	points.data()[10] = min[1];
	points.data()[11] = min[2];

	points.data()[12] = min[0];
	points.data()[13] = min[1];
	points.data()[14] = max[2];

	points.data()[15] = min[0];
	points.data()[16] = max[1];
	points.data()[17] = max[2];

	points.data()[18] = max[0];
	points.data()[19] = max[1];
	points.data()[20] = max[2];

	points.data()[21] = max[0];
	points.data()[22] = min[1];
	points.data()[23] = max[2];
...
void Floor::setTranslation(qeal x, qeal y, qeal z)
{
	for (int i = 0; i < pointsNum; i++)
	{
		points.buffer[3 * i + 0] += x;
