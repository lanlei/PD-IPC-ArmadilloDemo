...
void BaseMedialMesh::placeOrigin(const qeal lx, const qeal ly, const qeal lz)
{
	for (int i = 0; i < medialPointsNum; i++)
	{
		medialPoints.buffer[3 * i] -= lx;
		medialPoints.buffer[3 * i + 1] -= ly;
		medialPoints.buffer[3 * i + 2] -= lz;
	}
}
...
void BaseMedialMesh::scaleMesh(const qeal sx, const qeal sy, const qeal sz, const qeal cx, const qeal cy, const qeal cz)
{
	qeal sr = max3(sx, sy, sz);
	for (int i = 0; i < medialPointsNum; i++)
	{
		medialPoints.buffer[3 * i] = (medialPoints.buffer[3 * i] - cx) * sx + cx;
		medialPoints.buffer[3 * i + 1] = (medialPoints.buffer[3 * i + 1] - cy) * sy + cy;
		medialPoints.buffer[3 * i + 2] = (medialPoints.buffer[3 * i + 2] - cz) * sz + cz;
		medialRadius.buffer[i] *= sr;
	}
}
