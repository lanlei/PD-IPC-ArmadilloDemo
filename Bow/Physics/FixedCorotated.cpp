...
inline void dpsi_dsigma(const Eigen::MatrixBase<DerivedV>& sigma, const Scalar mu, const Scalar lam, Eigen::MatrixBase<DerivedW>& de_dsigma)
{
    ...
    T sigma_prod = sigma.prod();
    DerivedW sigma_prod_noI;
    if constexpr (dim == 2) {
    ...
    de_dsigma = T(2) * mu * (sigma - Bow::Vector<T, dim>::Ones()) + lam * (sigma_prod - T(1)) * sigma_prod_noI;
}
...
inline void B_left_coeff(const Eigen::MatrixBase<DerivedV>& sigma, const Scalar mu, const Scalar lam, Eigen::MatrixBase<DerivedW>& left_coeff)
{
    ...
    if constexpr (dim == 2)
        left_coeff[0] = mu - T(0.5) * lam * (sigma.prod() - T(1));
    else {
        T sigma_prod = sigma.prod();
...
BOW_INLINE Scalar psi(const Eigen::MatrixBase<DerivedM>& F, const Scalar mu, const Scalar lam)
{
    ...
    return mu * (sigma - Bow::Vector<T, dim>::Ones()).squaredNorm() + T(0.5) * lam * std::pow(sigma.prod() - T(1), 2);
}
...
BOW_INLINE void first_piola_differential(const Eigen::MatrixBase<DerivedM>& F, const Eigen::MatrixBase<DerivedM>& dF, Scalar mu, Scalar lam, Eigen::MatrixBase<DerivedM>& dP)
{
    ...
    if constexpr (!projectPD) {
        ...
        dP.noalias() = lam * JFinvT.cwiseProduct(dF).sum() * JFinvT;
        dP += 2 * mu * dF;
        MATH_TOOLS::addScaledRotationalDifferential(R, S, dF, -2 * mu, dP);
        MATH_TOOLS::addScaledCofactorMatrixDifferential(F, dF, lam * (J - (T)1), dP);
    }
    else {
        ...
        if constexpr (dim == 2) {
            ...
            T psi0 = _2mu * (sigma(0) - 1) + _lambda * Sprod[0];
            T psi1 = _2mu * (sigma(1) - 1) + _lambda * Sprod[1];
            T psi00 = _2mu + lam * Sprod[0] * Sprod[0];
            T psi11 = _2mu + lam * Sprod[1] * Sprod[1];
            T psi01 = _lambda + lam * Sprod[0] * Sprod[1];
            T m01 = _2mu - _lambda;
            ...
            T psi0 = _2mu * (sigma(0) - 1) + _lambda * Sprod[0];
            T psi1 = _2mu * (sigma(1) - 1) + _lambda * Sprod[1];
            T psi2 = _2mu * (sigma(2) - 1) + _lambda * Sprod[2];
            T psi00 = _2mu + lam * Sprod[0] * Sprod[0];
            T psi11 = _2mu + lam * Sprod[1] * Sprod[1];
            T psi22 = _2mu + lam * Sprod[2] * Sprod[2];
            T psi01 = _lambda * sigma(2) + lam * Sprod[0] * Sprod[1];
            T psi02 = _lambda * sigma(1) + lam * Sprod[0] * Sprod[2];
            T psi12 = _lambda * sigma(0) + lam * Sprod[1] * Sprod[2];
            T m01 = _2mu - _lambda * sigma(2); // i = 0
            T m02 = _2mu - _lambda * sigma(1); // i = 2
            T m12 = _2mu - _lambda * sigma(0); // i = 1
            T p01 = (psi0 + psi1) / clamp_small_magnitude(sigma(0) + sigma(1), 1e-6);
