...
namespace tinyply {
	...
	struct PlyProperty {
		BOW_INLINE PlyProperty(std::istream& is);
		BOW_INLINE PlyProperty(Type type, std::string& _name)
			: name(_name), propertyType(type) {}
		BOW_INLINE PlyProperty(Type list_type, Type prop_type, std::string& _name, size_t list_count)
	...
	struct PlyElement {
		BOW_INLINE PlyElement(std::istream& istream);
		BOW_INLINE PlyElement(const std::string& _name, size_t count)
	...
	struct PlyFile {
		...
		/*
			 ...
			 */
		BOW_INLINE std::vector<PlyElement> get_elements() const;
		BOW_INLINE std::vector<std::string> get_info() const;
		BOW_INLINE std::vector<std::string>& get_comments();
		BOW_INLINE bool is_binary_file() const;
