...
struct PlyFile::PlyFileImpl {
	...
	BOW_INLINE void read(std::istream& is);
	BOW_INLINE void write(std::ostream& os, bool isBinary);

	BOW_INLINE std::shared_ptr<PlyData> request_properties_from_element(const std::string& elementKey,
		const std::vector<std::string> propertyKeys,
		const uint32_t list_size_hint);

	BOW_INLINE void add_properties_to_element(const std::string& elementKey,
		const std::vector<std::string> propertyKeys,
		const Type type, const size_t count, const uint8_t* data, const Type listType, const size_t listCount);

	BOW_INLINE size_t read_property_binary(const size_t& stride, void* dest, size_t& destOffset, std::istream& is) noexcept;
	BOW_INLINE size_t read_property_ascii(const Type& t, const size_t& stride, void* dest, size_t& destOffset, std::istream& is);

	BOW_INLINE std::vector<std::vector<PropertyLookup>> make_property_lookup_table();

	BOW_INLINE bool parse_header(std::istream& is);
	BOW_INLINE void parse_data(std::istream& is, bool firstPass);
	BOW_INLINE void read_header_format(std::istream& is);
	BOW_INLINE void read_header_element(std::istream& is);
	BOW_INLINE void read_header_property(std::istream& is);
	BOW_INLINE void read_header_text(std::string line, std::vector<std::string>& place, int erase = 0);

	BOW_INLINE void write_header(std::ostream& os) noexcept;
	BOW_INLINE void write_ascii_internal(std::ostream& os) noexcept;
	BOW_INLINE void write_binary_internal(std::ostream& os) noexcept;
	BOW_INLINE void write_property_ascii(Type t, std::ostream& os, const uint8_t* src, size_t& srcOffset);
	BOW_INLINE void write_property_binary(std::ostream& os, const uint8_t* src, size_t& srcOffset, const size_t& stride) noexcept;
};
