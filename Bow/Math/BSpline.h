...
template <int interpolation_degree, class T> int baseNode(const T& x);
template <int interpolation_degree, class T, int dim> Vector<int, dim> baseNode(const Vector<T, dim>& x);
template <class T> void computeBSplineWeights(const T x, int& base_node, Vector<T, 1>& w, Vector<T, 1>* dw = 0, Vector<T, 1>* ddw = 0);
template <class T> void computeBSplineWeights(const T x, int& base_node, Vector<T, 2>& w, Vector<T, 2>* dw = 0, Vector<T, 2>* ddw = 0);
template <class T> void computeBSplineWeights(const T x, int& base_node, Vector<T, 3>& w, Vector<T, 3>* dw = 0, Vector<T, 3>* ddw = 0);
template <class T> void computeBSplineWeights(const T x, int& base_node, Vector<T, 4>& w, Vector<T, 4>* dw = 0, Vector<T, 4>* ddw = 0);

template <class T, int dim, int degree = 2>
...
template <class T, int dim, int degree = 2>
class BSplineWeightsWithSecondOrder {
    const T dx;
...
public:
    ...
    Vector<T, interpolation_degree + 1> w[dim];
    Vector<T, interpolation_degree + 1> dw[dim];
    Vector<T, interpolation_degree + 1> ddw[dim];
    const T one_over_dx;
    Vector<int, dim> base_node;

    BSplineWeightsWithSecondOrder(const Vector<T, dim>& X, T dx)
        : dx(dx), one_over_dx(1 / dx), base_node(Vector<int, dim>::Zero())
    {
        compute(X);
    }
    ...
    void compute(const Vector<T, dim>& X)
    {
        Vector<T, dim> X_index_space = one_over_dx * X;
        for (int d = 0; d < dim; d++)
            computeBSplineWeights(X_index_space[d], base_node[d], w[d], &dw[d], &ddw[d]);
    }
};
...
/**
\brief Compute constant weights
\param x the point in a h=1 grid
\param w the weights.
\param dw the weight gradients.
*/
template <class T>
inline void computeBSplineWeights(const T x, int& base_node, Vector<T, 1>& w, Vector<T, 1>* dw, Vector<T, 1>* ddw)
{
    base_node = baseNode<0>(x);
    w << 1;
    if (dw)
        (*dw) << 0;
    if (ddw)
...
*/
template <class T>
inline void computeBSplineWeights(const T x, int& base_node, Vector<T, 2>& w, Vector<T, 2>* dw, Vector<T, 2>* ddw)
{
...
*/
template <class T>
inline void computeBSplineWeights(const T x, int& base_node, Vector<T, 3>& w, Vector<T, 3>* dw, Vector<T, 3>* ddw)
{
...
*/
template <class T>
inline void computeBSplineWeights(const T x, int& base_node, Vector<T, 4>& w, Vector<T, 4>* dw, Vector<T, 4>* ddw)
{
