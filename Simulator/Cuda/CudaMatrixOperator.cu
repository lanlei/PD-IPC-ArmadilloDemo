...
__device__ __forceinline__
void scaleVector(qeal* v, qeal s, int dim)
{
	for (int i = 0; i < dim; i++)
		v[i] = s * v[i];
}
...
qeal getVectorNorm(qeal* v, int dim)
{
	...
	for (int i = 0; i < dim; i++)
		sum += v[i] * v[i];
	return CUDA_SQRT(sum);
}

__device__ __forceinline__
qeal getVectorSqNorm(qeal* v, int dim)
{
	qeal sum = 0.0;
	for (int i = 0; i < dim; i++)
		sum += v[i] * v[i];
...
__device__ __forceinline__
void getVectorNormalized(qeal* v, qeal* result, int dim)
{
	qeal len = getVectorNorm(v, dim);
	for (int i = 0; i < dim; i++)
		result[i] = v[i] / len;
...
__device__ __forceinline__
void getVectorAdd(qeal* v1, qeal* v2, qeal* result, const qeal s1, const qeal s2, int dim)
{
	for (int i = 0; i < dim; i++)
...
__device__ __forceinline__
void getVectorSub(qeal* v1, qeal* v2, qeal* result, const qeal s1, const qeal s2, int dim)
{
	for (int i = 0; i < dim; i++)
		result[i] = s1 * v1[i] - s2 * v2[i];
}
