...
void BaseRightWidget::pressMainLightAmbientBtn()
{
	...
	Q_EMIT openColorPanel(&(_scene->getLightSetting()->getPointLight()->ambientColor));
}
...
void BaseRightWidget::pressMainLightDiffuseBtn()
{
	...
	Q_EMIT openColorPanel(&(_scene->getLightSetting()->getPointLight()->diffuseColor));
}
...
void BaseRightWidget::pressMainLightSpecularBtn()
{
	...
	Q_EMIT openColorPanel(&(_scene->getLightSetting()->getPointLight()->specularColor));
}
...
void BaseRightWidget::refreshMeshGroup()
{
	...
	mesh_scale_x->setEnabled(valid);
	mesh_scale_y->setEnabled(valid);
	mesh_scale_z->setEnabled(valid);
	...
	mesh_scale_x->disconnect(SIGNAL(textChanged(const QString &)));
	mesh_scale_y->disconnect(SIGNAL(textChanged(const QString &)));
	mesh_scale_z->disconnect(SIGNAL(textChanged(const QString &)));
	if (!valid)
		return;

	mesh_hide_checkBox->setChecked(m->getSurfaceHandle()->getMesh()->isHide());
	qeal sx, sy, sz, tx, ty, tz, rx, ry, rz, rsita;
	m->getScale(sx, sy, sz);
	...
	mesh_scale_x->setText(QString::number(sx));
	mesh_scale_y->setText(QString::number(sy));
	mesh_scale_z->setText(QString::number(sz));
	...
	connect(mesh_scale_x, SIGNAL(textChanged(const QString &)), this, SLOT(setMeshScale()));
	connect(mesh_scale_y, SIGNAL(textChanged(const QString &)), this, SLOT(setMeshScale()));
	connect(mesh_scale_z, SIGNAL(textChanged(const QString &)), this, SLOT(setMeshScale()));
}
...
void BaseRightWidget::setMeshScale()
{
	...
	if (m == nullptr) return;
	qeal sx = mesh_scale_x->text().toDouble();
	if (sx <= 0) sx = 1.0;
	qeal sy = mesh_scale_y->text().toDouble();
	if (sy <= 0) sy = 1.0;
	qeal sz = mesh_scale_z->text().toDouble();
	if (sz <= 0) sz = 1.0;
